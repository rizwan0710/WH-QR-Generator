import os
import tkinter as tk
from tkinter import messagebox, filedialog
from tkinter import ttk
import re
import sqlite3
from PIL import Image, ImageTk

def dapatkan_part_no_dari_db(inner_seq):
    try:
        with sqlite3.connect("warehouse_data.db", timeout=10) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT part_no FROM rekod_qr WHERE sequence_no = ?", (inner_seq,))
            row = cursor.fetchone()
            if row and row[0]:
                return str(row[0]).strip()
    except Exception:
        pass
    return inner_seq  

def cetak_qr_outer_wizard_lokal(img_label):
    try:
        temp_file = "temp_print_outer_wizard.png"
        img_label.save(temp_file)
        import sys
        if sys.platform == "win32":
            os.startfile(temp_file, "print")
    except Exception as e:
        print(f"Print error: {str(e)}")

def simpan_qr_manual_wizard_lokal(img_label, seq_outer):
    if img_label is None: return
    fail_clean = str(seq_outer).replace("/", "-").replace(":", "-").strip()
    path_simpan = filedialog.asksaveasfilename(
        initialfile=f"REPRINT_OUTER_{fail_clean}.png",
        defaultextension=".png",
        filetypes=[("PNG Files", "*.png")],
        title="Simpan Label Outer Box"
    )
    if path_simpan:
        try:
            img_label.convert("RGB").save(path_simpan, "PNG")
            messagebox.showinfo("Success", f"Label Outer Box [{fail_clean}] Successfully Saved!")
        except Exception as e:
            messagebox.showerror("STORAGE ERROR", str(e))

def buka_popup_pukal_outer_1by1(item_ditanda, root):
    """🌟 BATCH OUTER WIZARD ENGINE (LANDSCAPE - DYNAMIC STANDARDIZED) 🌟"""
    tingkap_popup = tk.Toplevel(root)
    tingkap_popup.title("OUTER BOX BATCH PANEL")
    tingkap_popup.geometry("540x510+420+120") 
    tingkap_popup.configure(bg="#F8F9FA")
    tingkap_popup.grab_set()

    import label_outer_designer as lod
    import qrcode

    senarai_kad_pembungkus = []
    
    for r in item_ditanda:
        try:
            seq_outer = str(r[8]).strip()
            raw_inner_text = str(r[6]).strip()
        except IndexError:
            continue
        
        senarai_inner_bersih = [s.strip() for s in re.split(r'[,\n]', raw_inner_text) if s.strip() and s.strip() != "None"]
        
        data_gabungan_final = []
        for s_clean in senarai_inner_bersih:
            if s_clean:
                part_no_asli = dapatkan_part_no_dari_db(s_clean)
                q_val = "0"
                try:
                    with sqlite3.connect("warehouse_data.db", timeout=10) as conn:
                        cursor = conn.cursor()
                        cursor.execute("SELECT quantity FROM rekod_qr WHERE sequence_no = ?", (s_clean,))
                        row_q = cursor.fetchone()
                        if row_q and row_q[0]:
                            q_val = str(row_q[0])
                except Exception:
                    pass
                
                data_gabungan_final.append((part_no_asli, str(q_val)))
                
        qr = qrcode.QRCode(version=1, box_size=10, border=1)
        qr.add_data(seq_outer)
        qr.make(fit=True)
        img_qr_mentah = qr.make_image(fill_color="black", back_color="white").convert("RGB")
        
        img_kad = lod.bina_imej_gabungan_akses(img_qr_mentah, data_gabungan_final, seq_outer)
        senarai_kad_pembungkus.append((img_kad, seq_outer))

    indeks_halaman = 0
    total_label = len(senarai_kad_pembungkus)

    lbl_header = tk.Label(tingkap_popup, text="", font=("Segoe UI", 10, "bold"), fg="#0078D7", bg="#F8F9FA")
    lbl_header.pack(pady=12)

    frame_canvas_bg = tk.Frame(tingkap_popup, bg="white", bd=1, relief="groove")
    frame_canvas_bg.pack(fill=tk.BOTH, expand=True, padx=30, pady=5)
    
    label_gambar = tk.Label(frame_canvas_bg, bg="white")
    label_gambar.pack(padx=15, pady=15, expand=True, fill=tk.BOTH)

    def kemaskini_paparan_selak():
        if total_label == 0: return
        img_kad, seq_outer = senarai_kad_pembungkus[indeks_halaman]
        lbl_header.config(text=f"OUTER PREVIEW ({seq_outer})  |  BATCH COUNTER: {indeks_halaman + 1}/{total_label}")
        
        img_visual = img_kad.resize((420, 200), Image.Resampling.LANCZOS)
        img_tk = ImageTk.PhotoImage(img_visual)
        label_gambar.config(image=img_tk)
        label_gambar.image = img_tk 
        
        if total_label > 1:
            btn_prev.config(state="normal" if indeks_halaman > 0 else "disabled")
            btn_next.config(state="normal" if indeks_halaman < total_label - 1 else "disabled")

    def halaman_ke_kiri():
        nonlocal indeks_halaman
        if indeks_halaman > 0:
            indeks_halaman -= 1
            kemaskini_paparan_selak()

    def halaman_ke_kanan():
        nonlocal indeks_halaman
        if indeks_halaman < total_label - 1:
            indeks_halaman += 1
            kemaskini_paparan_selak()

    def cetak_semua_pukal():
        if messagebox.askyesno("CONFIRMATION", f"Print ALL {total_label} Outer Box label?", parent=tingkap_popup):
            for img_kad, _ in senarai_kad_pembungkus: 
                cetak_qr_outer_wizard_lokal(img_kad)
            messagebox.showinfo("SUCCESS", "All Outer Box label received to printer!", parent=tingkap_popup)

    def simpan_semua_pukal():
        folder_tujuan = filedialog.askdirectory(title="CHOOSE FOLDER TO SAVE", parent=tingkap_popup)
        if folder_tujuan:
            for img_kad, seq_outer in senarai_kad_pembungkus:
                img_kad.convert("RGB").save(os.path.join(folder_tujuan, f"OUTER_{seq_outer}.png"), "PNG")
            messagebox.showinfo("SUCCESS", "All Outer Box label successfully saved!", parent=tingkap_popup)

    def simpan_tunggal_sahaja():
        img_kad, seq_outer = senarai_kad_pembungkus[indeks_halaman]
        simpan_qr_manual_wizard_lokal(img_kad, seq_outer)

    # ─── 1. BAR NAVIGASI (Hanya pack jika data > 1) ───
    frame_nav = tk.Frame(tingkap_popup, bg="#F8F9FA")
    btn_prev = tk.Button(frame_nav, text="◀ PREVIOUS", command=halaman_ke_kiri, bg="#374151", fg="white", font=("Segoe UI", 9, "bold"), width=13, relief="flat", cursor="hand2")
    btn_prev.pack(side=tk.LEFT, padx=8)
    btn_next = tk.Button(frame_nav, text="NEXT ▶", command=halaman_ke_kanan, bg="#374151", fg="white", font=("Segoe UI", 9, "bold"), width=13, relief="flat", cursor="hand2")
    btn_next.pack(side=tk.LEFT, padx=8)

    if total_label > 1:
        frame_nav.pack(pady=5)

    # ─── 2. ACTION BAR MULTIPLE VIEW SERAGAM OHTA PRECISION (DINAMIK) ───
    frame_btn = tk.Frame(tingkap_popup, bg="#F8F9FA")
    frame_btn.pack(pady=15, side=tk.BOTTOM, fill=tk.X, padx=20)
    
    btn_style = {"font": ("Segoe UI", 9, "bold"), "fg": "white", "relief": "flat", "height": 2, "cursor": "hand2"}
    
    if total_label == 1:
        tk.Button(frame_btn, text="🖨️ PRINT ", command=lambda: cetak_qr_outer_wizard_lokal(senarai_kad_pembungkus[0][0]), bg="#22C55E", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(frame_btn, text="💾 SAVE ", command=simpan_tunggal_sahaja, bg="#F59E0B", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(frame_btn, text="❌ CLOSE", command=tingkap_popup.destroy, bg="#374151", **btn_style).pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=4)
    else:
        tk.Button(frame_btn, text="🖨️ PRINT CURRENT", command=lambda: cetak_qr_outer_wizard_lokal(senarai_kad_pembungkus[indeks_halaman][0]), bg="#22C55E", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        tk.Button(frame_btn, text="🔥 PRINT ALL", command=cetak_semua_pukal, bg="#10B981", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        tk.Button(frame_btn, text="💾 SAVE CURRENT", command=simpan_tunggal_sahaja, bg="#F59E0B", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        tk.Button(frame_btn, text="📦 SAVE ALL", command=simpan_semua_pukal, bg="#EA580C", **btn_style).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        tk.Button(frame_btn, text="❌ CLOSE", command=tingkap_popup.destroy, bg="#374151", **btn_style).pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=2)

    kemaskini_paparan_selak()
