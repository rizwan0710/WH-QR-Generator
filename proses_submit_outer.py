def proses_submit_outer(win_outer, entry_date, entries_inner):
    ...
    # Semak sekatan penduaan (Isu ke-2)
    ...
    # Semak silang percampuran data customer (Isu ke-1)
    for idx, seq in enumerate(senarai_inner, start=1):
        ...
        if cust != customer_utama:
            messagebox.showerror(
                ...
                f"• SLOT 1 (Utama): {customer_utama}\n"
                f"• SLOT {idx} (Ralat): {cust} (Siri: {seq})\n\n"
                f"PLEASE ENSURE THAT ALL SLOTS BELONG TO SAME COMPANY!", 
                parent=win_outer
            )
            return
    ...
    try:
        ...
        paparkan_pop_up_imej_label(win_outer, entries_inner, img_label_final, "OUTER PACKING LABEL GENERATED", sequence_outer_final)
        
    except Exception as e:
        messagebox.showerror("DATABASE ERROR", f"FAILED TO EXECURE OUTER BOX REGISTRY:\n{str(e)}", parent=win_outer)
